...
func TestTimedInvokeParity(t *testing.T) {
	...
	// matter.js is a sibling checkout one level above the module root. The
	// root is found by walking up to go.mod rather than by counting parent
	// directories, so moving this file cannot silently point the lookup at
	// an unrelated tree.
	elementsDir := filepath.Join(moduleRoot(t), "..", "matter.js", "packages", "model", "src", "standard", "elements")
