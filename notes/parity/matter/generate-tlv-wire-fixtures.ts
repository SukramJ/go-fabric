// Generates wire-byte fixtures for the go-fabric TLV encoder to match
...
// TlvCodec.encode() output — go-fabric's encoder must produce the
...
// type combinations go-fabric's encoder must produce.
...
out.push({
    label: "utf8_anon_go-fabric",
    description: "PutUTF8(AnonymousTag, 'go-fabric')",
    bytesHex: bytesToHex(encodeOne({ type: TlvType.Utf8String, length: 0 }, ANON, "go-fabric")),
});
