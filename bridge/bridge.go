...
type Bridge struct {
	...
	// and chip both drop these datagrams silently — go-fabric does
...
func (b *Bridge) Start(ctx context.Context) error {
	...
	// Access control is NOT checked here. The daemon this module was
	// extracted from attaches its session lookup after Start returns, which
	// is a legitimate order the bridge supports — so a start-time check
	// refuses correct wiring while still missing anything attached later.
	// The question is asked where it is answered instead, in
	// resolveSessionFabric: a session whose fabric cannot be named resolves
	// to endpoint.FabricIndexUnresolvable rather than to 0, and CheckACL
	// denies that instead of reading it as commissioning traffic.
...
func (b *Bridge) reassembleLocked(ctx context.Context) error { //nolint:gocognit,funlen // single-purpose bridge topology reassembly with many endpoint/cluster branches
	...
	//
	// In go-fabric the state is not reactive (Go structs have no
