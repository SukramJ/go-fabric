...
func TestAttachClustersConcurrentWithLiveRead(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		for i := range rounds {
			b.AttachRootClusters([]contract.ClusterServer{&noopCluster{id: 0x0028 + uint32(i%2)}})
			b.AttachAggregatorClusters([]contract.ClusterServer{&noopCluster{id: 0x001D}})
		}
	}()
	...
	b.AttachRootClusters([]contract.ClusterServer{&noopCluster{id: 0x0028}})
	b.AttachAggregatorClusters([]contract.ClusterServer{&noopCluster{id: 0x001D}})
