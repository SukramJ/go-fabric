...
// go-fabric's TLV encoder and matter.js produces silent Apple Home
...
// encode runs the go-fabric encoder for a single label and returns
...
func encode(t *testing.T, label string) []byte {
	...
	case "utf8_anon_go-fabric":
		enc.PutUTF8(AnonymousTag(), "go-fabric")
	...
	// resolves before encoding). go-fabric emits them so cluster code
...
// TestParityMatterJS_EncoderWireBytes verifies that go-fabric's TLV
