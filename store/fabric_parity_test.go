...
//   t.Skip-annotated: go-fabric's store layer stores raw blobs and does
...
// CHIP_ERROR_INVALID_FABRIC_INDEX; in go-fabric, FabricIndex==0 is the
...
//
// go-fabric gap: label-length enforcement lives in the
...
func TestParityChip_FabricLabel_TooLongFails(t *testing.T) {
	t.Skip("FixMe: go-fabric gap — label-length enforcement is in OperationalCredentials cluster, not store layer")
}
...
// is the callback; go-fabric's equivalent is verified in the cluster layer
...
// immediately (go-fabric has no staged-commit model like chip; every write
...
// commit/rollback; go-fabric uses transactional immediate writes.
...
func TestParityChip_AddFabric_ChipStagedCommit_NotApplicable(t *testing.T) {
	t.Skip("FixMe: go-fabric gap — chip staged-commit (pending→commit/rollback) has no equivalent; go-fabric writes are immediately durable per-transaction")
}
