...
func TestGetExposure_NotFound(t *testing.T) {
	...
	db := openTestDB(t)
	s := store.New(db)
	_, err := s.GetExposure(context.Background(), testKey("c1", "X:1", 1, store.DPKindCustom, "K"))
	if !errors.Is(err, store.ErrExposureNotFound) {
...
func TestGetExposure_Found(t *testing.T) {
	...
	key := testKey("c1", "X:1", 1, store.DPKindCustom, "K")
	rec := store.ExposureRecord{Key: key, Enabled: true, FriendlyName: "My Lamp", Actor: "user"}
...
func TestIsExposed_Missing_ReturnsFalse(t *testing.T) {
	...
	db := openTestDB(t)
	s := store.New(db)
	ok, err := s.IsExposed(context.Background(), testKey("c1", "Y:1", 1, store.DPKindCustom, "K"))
	if err != nil {
...
func TestIsExposed_DisabledRow_ReturnsFalse(t *testing.T) {
	...
	key := testKey("c1", "Y:2", 1, store.DPKindCustom, "K")
	if err := s.UpsertExposure(ctx, store.ExposureRecord{Key: key, Enabled: false, Actor: "sys"}); err != nil {
		t.Fatalf("UpsertExposure: %v", err)
	}
...
func TestIsExposed_EnabledRow_ReturnsTrue(t *testing.T) {
	...
	key := testKey("c1", "Y:3", 1, store.DPKindCustom, "K")
...
func TestUpsertExposure_UpdateEnabled(t *testing.T) {
	...
	key := testKey("c1", "R:1", 1, store.DPKindCustom, "T")
	if err := s.UpsertExposure(ctx, store.ExposureRecord{Key: key, Enabled: false, Actor: "u"}); err != nil {
...
func TestDeleteExposure_Existing(t *testing.T) {
	...
	key := testKey("c1", "S:1", 1, store.DPKindCustom, "D")
	if err := s.UpsertExposure(ctx, store.ExposureRecord{Key: key, Enabled: true, Actor: "u"}); err != nil {
		t.Fatalf("UpsertExposure: %v", err)
	}
...
func TestDeleteExposure_Missing_NoError(t *testing.T) {
	...
	db := openTestDB(t)
	s := store.New(db)
	key := testKey("c1", "T:1", 1, store.DPKindCustom, "E")
...
func TestGetExposure_DisabledRow(t *testing.T) {
	...
	key := testKey("c1", "XX:1", 1, store.DPKindCustom, "DISABLED")
	if err := s.UpsertExposure(ctx, store.ExposureRecord{Key: key, Enabled: false, Actor: "sys"}); err != nil {
		t.Fatalf("UpsertExposure: %v", err)
	}
	got, err := s.GetExposure(ctx, key)
...
func TestListExposures_Mixed(t *testing.T) {
	...
	keys := []store.EndpointKey{
		testKey("mix", "M:1", 1, store.DPKindCustom, "A"),
		testKey("mix", "M:2", 2, store.DPKindCustom, "B"),
	}
	for i, k := range keys {
		if err := s.UpsertExposure(ctx, store.ExposureRecord{Key: k, Enabled: i%2 == 0, Actor: "u"}); err != nil {
...
func TestCountEnabled_Global(t *testing.T) {
	...
	disKey := testKey("n1", "V:3", 3, store.DPKindCustom, "K")
	if err := s.UpsertExposure(ctx, store.ExposureRecord{Key: disKey, Enabled: false, Actor: "u"}); err != nil {
...
func TestExposures_EnabledKeys_OnlyEnabled(t *testing.T) {
	...
	for i, enabled := range []bool{true, false, true} {
		key := testKey("ek2", "A:1", i+2, store.DPKindCustom, "K")
		if err := s.UpsertExposure(ctx, store.ExposureRecord{Key: key, Enabled: enabled, Actor: "u"}); err != nil {
			t.Fatalf("UpsertExposure[%d]: %v", i, err)
		}
	}
...
func TestExposures_GetMissReturnsNotFound(t *testing.T) {
	...
	key := testKey("miss", "X:1", 1, store.DPKindCustom, "MISSING")
	_, err := s.GetExposure(ctx, key)
	if !errors.Is(err, store.ErrExposureNotFound) {
...
func TestExposures_IsExposedMissReturnsFalse(t *testing.T) {
	...
	key := testKey("iexp", "Y:2", 3, store.DPKindGeneric, "STATE")
	ok, err := s.IsExposed(ctx, key)
	if err != nil {
		t.Fatalf("IsExposed: %v", err)
	}
...
func TestExposures_IsExposedDisabledReturnsFalse(t *testing.T) {
	...
	key := testKey("dis", "D:1", 1, store.DPKindCustom, "K")
	if err := s.UpsertExposure(ctx, store.ExposureRecord{Key: key, Enabled: false, Actor: "test"}); err != nil {
		t.Fatalf("UpsertExposure: %v", err)
	}
	ok, err := s.IsExposed(ctx, key)
	if err != nil {
...
func TestExposures_DeleteIdempotent(t *testing.T) {
	...
	key := testKey("del", "Z:1", 2, store.DPKindCalculated, "V")
...
func TestExposures_ListNoFilter(t *testing.T) {
	...
	for i := range 4 {
		key := testKey("lall", "A:1", i+1, store.DPKindCustom, "K")
		if err := s.UpsertExposure(ctx, store.ExposureRecord{Key: key, Enabled: i%2 == 0, Actor: "t"}); err != nil {
			t.Fatalf("UpsertExposure[%d]: %v", i, err)
		}
	}
