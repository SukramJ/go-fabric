...
!!! info "Who this page is for"
    Contributors and AI agents working on the Matter side of a bridge. The
    parity rules are the
    [Matter Behavioural-Parity Contract](./matter-parity-contract.md); an
    operator-facing guide belongs to the host application (OpenCCU-Loom keeps
    one at [`docs/user/matter.md`](https://github.com/SukramJ/openccu-loom/blob/main/docs/user/matter.md)).
...
A source path beginning `internal/` is
[OpenCCU-Loom](https://github.com/SukramJ/openccu-loom)'s: the observation was
made against that daemon while this module still lived inside it, and the
device projection it names is still the host's. Everything else is a path in
this repository.
...
| Missing `BridgedDeviceBasicInformation` or `Descriptor` on a bridged endpoint aborts the pair with `HMMTRErrorDomain Code 9` ("no topology / no link layer") roughly 9 s after CommissioningComplete. Empty fields inside `BridgedDeviceBasicInformation` surface as `HAPErrorDomain Code=24`. | Both clusters are mounted unconditionally, with VendorName / ProductName / SerialNumber / UniqueID populated. | Reproduced | go-fabric [`endpoint/materialize.go`](https://github.com/SukramJ/go-fabric/blob/main/endpoint/materialize.go) |
| The post-CommissioningComplete flow writes `AccessControl.ACL` to install HomePod / Apple TV edge controllers as additional Administer subjects. Without a decoder for that structured write the WriteResponse never lands, Apple times out after 10 s and tears down the fabric via RemoveFabric. | Every writable list/struct attribute needs an explicit decoder case; a fall-through reaches `MatterWrite` as nil. | Reproduced | go-fabric [`bridge/attribute_value_reader.go`](https://github.com/SukramJ/go-fabric/blob/main/bridge/attribute_value_reader.go) |
| A `CountryCode` silently decoded as `""` had Apple sending RemoveFabric about 80 s into the pair. | UTF-8 TLV fields are read from `el.String`, never `el.Octets`. | Reproduced | go-fabric [`bridge/fields_reader.go`](https://github.com/SukramJ/go-fabric/blob/main/bridge/fields_reader.go) |
| Cancelling a pairing in Apple Home and retrying returns BUSY for up to 900 s when the aborted attempt left the fail-safe armed. | `RevokeCommissioning` expires the fail-safe unconditionally, before the window-state check. | Reproduced | go-fabric [`bridge/commissioning_window.go`](https://github.com/SukramJ/go-fabric/blob/main/bridge/commissioning_window.go) |
| Parallel CASE sessions are opened from one controller, and Sigma3 is retransmitted by the MRP layer. | Sigma2 replies are deduplicated per exchange ID. | Reproduced | go-fabric [`bridge/bridge.go`](https://github.com/SukramJ/go-fabric/blob/main/bridge/bridge.go), [`bridge/case_provider.go`](https://github.com/SukramJ/go-fabric/blob/main/bridge/case_provider.go) |
| Per-boot rotation of `UniqueID` breaks the HAP-service mapper's device identity across restarts. | UniqueID is stable across restarts; rotation exists only as a deliberate debug escape. | Reproduced | go-fabric [`bootid/bootid.go`](https://github.com/SukramJ/go-fabric/blob/main/bootid/bootid.go) |
