# ADR 0006 — Extract cohesive sub-helpers from `handleSubscribeRequest`

> **Provenance.** Moved here from OpenCCU-Loom, where this decision was
> recorded as ADR 0039 while the Matter stack still lived in that
...
- **Date**: 2026-06-16
- **Related**:
  `bridge/subscribe.go` (`handleSubscribeRequest`),
  `bridge/subscribe_dispatch.go`,
...
- Behaviour is identical — locked by the existing end-to-end subscribe
  ...
  `scenario_test.go`) plus `go test -race ./bridge/`
  passing unchanged.
