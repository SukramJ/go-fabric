# ADR 0002 — Extract a testable gate + per-opcode seam from `handleIMOpcode`

> **Provenance.** Moved here from OpenCCU-Loom, where this decision was
> recorded as ADR 0031 while the Matter stack still lived in that
...
- **Date**: 2026-06-15
- **Related**:
  `bridge/receive.go` (`handleIMOpcode`),
  `bridge/receive_test.go`,
