...
// (172.30.232.1) or a `docker0` address cannot route to the bridge from
...
// not matched).
//
// A host that advertises the same machine over DNS-SD for its own purposes
// will carry its own copy of this policy. Both then publish A-records for
// one host, so a divergence makes one announce an address the other
// deliberately suppresses. This module cannot see the host's copy, so
// pinning the two against each other is the host's job.
