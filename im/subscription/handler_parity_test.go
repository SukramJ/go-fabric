...
//
// Conversion notes:
//   - matter.js uses per-subscription timer objects; go-fabric uses a
...
func TestHandlerParity_PeerCancel_RemovesSubscriptionFromActive(t *testing.T) {
	...
	// Simulate peer cancellation via ClosePeer (the go-fabric counterpart
...
//
// In go-fabric there is no per-subscription exchange object; the
...
// go-fabric equivalent: markDirty called before the first Tick → dirty paths
...
// has it). In go-fabric the equivalent invariant is that an attribute change
...
// async Promise chain in matter.js; go-fabric's Tick is synchronous and the
...
// #sendUpdate which wraps the exchange messenger; go-fabric's Reporter is
...
func TestHandlerParity_SKIP_SendUpdateRetryCount(t *testing.T) {
	...
	t.Skip("FixMe: go-fabric gap — send-error retry count (≤2 re-queue, cancel on 3rd) lives in the transport layer above the engine; drift L10-Dxx-FUTURE")
}
...
// requires a live MockServerNode + protocol stack. The go-fabric equivalent
...
func TestHandlerParity_SKIP_AttrSubResponse_ConcreteInFilter(t *testing.T) {
	// Mirrors matter.js packages/node/test/node/AttributeSubscriptionResponseTest.ts:16-41
	t.Skip("FixMe: AttributeSubscriptionResponse requires a full IM+Node stack; go-fabric dirty-filter logic tested at the engine layer via drainDirtyIfElapsed")
}
...
// go-fabric must propagate those paths so switch-press events flow to the
