...
func validBasicInfoConfig() core.Config {
	return core.Config{
		VendorID:    0x1234,
		ProductID:   0x5678,
		NodeLabel:   "go-fabric-bridge",
		VendorName:  "go-fabric",
...
func TestBasicInfo_ReadAllAttributes(t *testing.T) {
	...
	// bridge sample does not emit them on Root, and go-fabric mirrors
...
func (f *fakeEventEmitter) MatterEmitEvent(ep uint16, clID, evID uint32, payload any, pri contract.EventPriority) {
	f.endpoint = ep
	f.clusterID = clID
	f.eventID = evID
	f.payload = payload
...
func TestBasicInfo_EmitReachableChanged(t *testing.T) {
	...
	if em.priority != contract.EventPriorityInfo {
