...
//     infrastructure that has no direct Go equivalent in go-fabric's
...
// go-fabric uses SetServerListProvider closure (Bug K fix).
...
// go-fabric's unit-test layer. They are covered at the integration level
// instead.
//
// Mirrors matter.js packages/node/test/behaviors/descriptor/
...
func TestParityMatterJS_DescriptorServer_AsyncPartsListUnsupported(t *testing.T) {
	t.Skip("FixMe: go-fabric gap — async MockServerNode lifecycle has no unit-test equivalent; covered by integration tests")
}
