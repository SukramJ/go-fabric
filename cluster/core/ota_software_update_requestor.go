...
//
// go-fabric v1.1 ships an explicit stub: OTA is delivered through
...
// TCP/UDP, which go-fabric does not currently implement.
...
// (the host's root-cluster composition does not mount
...
func (o *OTASoftwareUpdateRequestor) MatterRead(attrID uint32) (any, bool) {
	...
	case otaRequestorAttrUpdatePossible:
		// false — Matter OTA is not the go-fabric update path.
		return false, true
