...
// the matter.js element definition default (FeatureMap=0): go-fabric
...
func TestParityMatterJS_AccessControl_FeatureMapExtsSet(t *testing.T) {
	...
	if got&featureEXTS == 0 {
		t.Errorf("FeatureMap = 0x%08X, want EXTS bit (0x0001) set — go-fabric serves Extension list so the feature bit must be advertised", got)
	}
}
...
// feature; go-fabric must list both in MatterEvents() so that the
